...
def load_file(i):
    train_file = "D:\Pycharm\Pycharm_Projects\deepfm_pairwise\ccle\cgp_data\con_data/train_data/train_data" + str(
        i) + ".csv"
    print(train_file)
    train_df = pd.read_csv(train_file)
    train_df.rename(columns={'Unnamed: 0': 'id1'}, inplace=True)
    train_df.drop(columns="Unnamed: 0_y", inplace=True)
    train_df.drop_duplicates(subset=['id'], keep='last', inplace=True)  # 重复id（key）直接覆盖掉，
...
def gen_pairs_hard( y_id):
    # print("gen_pairs start: *******************")
    ## pair is too much，we chose to ignore the （AB，AC）, just save the （AB，CD）with unique drug.
    cannot_dict = config.cannot_hard_dict
    ...
    for i in range(0, len(y_id)):
        ...
        for j in range(i + 1, len(y_id)):
            ...
            if (y_id[i][0] == y_id[j][0] and y_id[i][1] != y_id[j][1]): ## 相同cell-id，不同的drug_id
                ...
                cannot_d =  cannot_dict[y_id[i][1]]
                # print(cannot_d)
                if y_id[j][1] in cannot_d:
                    # print(" *********** cannot be pairs 1 ********** ")
                    continue
                ...
                if (y_id[i][2] > y_id[j][2]):
                    # print(" *********** can be pairs ********** ")
                    pairs.append([y_id[i][0], int(i), int(j)]) #ids[i][0] 是cell-id  i,j 是针对 ids的序号，

                else:
                    pairs.append([y_id[i][0],int(j), int(i)])

    print(" original_count = %d"%origal_count)
    print(" hard_count = %d"%hard_count)
    return pairs
