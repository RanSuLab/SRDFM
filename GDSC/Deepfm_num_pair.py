...
from DataLoader_num import gen_pairs, gen_pairs_, gen_pairs_by_cell_line
...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, cate_feature_size, field_size, numeric_feature_size,
                 ...
                 epoch=1, batch_size=1024, mid_size = 10000,
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            if self.use_attention:
                ...
                self.attention_o2 = tf.add(tf.matmul(tf.reshape(self.embeddings_o2,
                                                   shape=[-1, self.field_size * self.embedding_size]), self.weights["attention_w"]),
                                           self.weights["attention_b"])
            ...
            ## 预测层加上BN
            #concat_input_o1 = tf.layers.batch_normalization(concat_input_o1, training=self.train_phase)
            #concat_input_o2 = tf.layers.batch_normalization(concat_input_o2, training=self.train_phase)
...
#####################out = w*X + b
    ...
    def _initialize_weights(self):
        ...
        weights['attention_w'] = tf.get_variable(name='attention_w', shape=[self.field_size * self.embedding_size,
                                                                            self.numeric_feature_size],
                                                 dtype=tf.float32, initializer=variance_scaling_initializer())

        weights['attention_b'] = tf.get_variable(name='attention_b', shape=[1, self.numeric_feature_size],
                                                 dtype=tf.float32, initializer=variance_scaling_initializer())
    ...
    def predict_part_train(self, data_parser, exp,fig,y_id,part = 1):
    ...
    def predict_pair_train(self, data_parser, exp,fig,y_id):
        ...
        for batch_index in range(total_batch):
            ...
            ## 每个batch内部得到pairs_batch
            y_id_ = self.get_batch(y_id, self.batch_size, batch_index)
            # ids_train_ = self.get_batch(ids_train, self.batch_size, batch_index)
            ...
            ## 得到 cate_Xi_batch_o1 ， cate_Xv_batch_o1 ，numeric_Xv_batch_o1
            # id_list
            ...
            # pairs_match_o1 = y_id_train_.loc[y_id_train_["id"] in id_list_o1] ## id有重复的，这样取不出来完整的
            # pairs_match_o2 = y_id_train_.loc[y_id_train_["id"] in id_list_o2]
            ...
            # print("merge 后的 batchTrain 数目：")
            # print(len(batchTrain1))
            # print(len(batchTrain2))
    ...
    def fit_on_batch(self,Xi_o1,Xv_o1,Xv2_o1,y_o1,Xi_o2,Xv_o2,Xv2_o2,y_o2):
        ...
        loss, o12, h_o12,a, optimizer,projection = self.sess.run([self.loss, self.o12,self.h_o12,self.a,self.optimizer, self.weights['concat_projection']], feed_dict=feed_dict)
    ...
    def fit(self, data_parser, exp, fig, y_id_train, y_id_valid=None,
            ...
            early_stopping=True, fold = 0, path = None):
        ...
        self.shuffle_in_unison_scary(y_id_train)
        self.shuffle_in_unison_scary(y_id_valid)
        ...
        for epoch in range(self.epoch):
            ...
            for i in range(total_batch):
                ...
                if len(batchTrain1) == len(id_list_o1):
                    ...
                    print(" Deepfm_num_pair.py row1786 : batchTrain merge wrong!")
                    exit()
            ...
            if has_valid:
                print("________________ evaluate_valid _______________________")

                valid_result = self.evaluate_pair_train(data_parser, exp,fig,y_id_valid)
                ...
                valid_predict_gini = precision_results_cv[0]  # k=1

                print(precision_results_cv)
                print(ndcg_results_cv)
                print(percentile_new_results_cv)
            ...
            if self.verbose > 0 and epoch % self.verbose == 0:
                ...
                if has_valid:
                    if epoch % 1 == 0:
                        ...
                        print(" [%d] - no evalate to save time! - [%.1f s] "
                               % (epoch +1 ,  time() - t1))
                else:
